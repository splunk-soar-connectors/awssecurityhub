...
class AwsSecurityHubConnector(BaseConnector):
    ...
    def initialize(self):
        ...
        if not isinstance(self._state, dict):
            ...
            return self.set_status(phantom.APP_ERROR, AWSSECURITYHUB_STATE_FILE_CORRUPT_ERROR)
    ...
    def _create_client(self, action_result, service="securityhub", param=None):
        ...
        try:
            if self._access_key and self._secret_key:
                ...
                self._client = client(service, region_name=self._region, config=boto_config)
    ...
    def _create_container(self, finding):
        ...
        if phantom.is_fail(container_creation_status):
            ...
            self.save_progress(
                "Error while creating container for finding {finding_id}. {error_message}".format(
                    finding_id=finding["Id"], error_message=container_creation_message
                )
            )
            return None
    ...
    def _handle_get_findings(self, param):
        ...
        if resource_id:
            filters.update({"ResourceId": [{"Value": resource_id, "Comparison": AWSSECURITYHUB_EQUALS_CONSTS}]})

        if is_archived:
            filters.update({"RecordState": [{"Value": "ARCHIVED", "Comparison": AWSSECURITYHUB_EQUALS_CONSTS}]})
        else:
            filters.update({"RecordState": [{"Value": "ACTIVE", "Comparison": AWSSECURITYHUB_EQUALS_CONSTS}]})
        ...
        if resource_ec2_ipv4_addresses:
            ...
            filters.update({"ResourceAwsEc2InstanceIpV4Addresses": ip_add_list})
        ...
        if network_source_ipv4:
            ...
            filters.update({"NetworkSourceIpV4": ip_add_list})

        if network_source_mac:
            filters.update({"NetworkSourceMac": [{"Value": network_source_mac, "Comparison": AWSSECURITYHUB_EQUALS_CONSTS}]})

        if resource_region:
            filters.update({"ResourceRegion": [{"Value": resource_region, "Comparison": AWSSECURITYHUB_EQUALS_CONSTS}]})
            ...
            if finding.get("Id") == findings_id:
                if record_state and finding.get("RecordState") == record_state:
                    action_result.set_status(
                        phantom.APP_SUCCESS, AWSSECURITYHUB_ERROR_FINDING_ID_IN_RECORD_STATE.format(record_state=record_state)
                    )
    ...
    def _handle_archive_findings(self, param):
        ...
        if note:
            ...
            note1 = {"Text": "(Splunk SOAR - {} time is {}) {}".format("Archived updated", note_time, note), "UpdatedBy": "automation-splunk"}
    ...
    def _handle_unarchive_findings(self, param):
        ...
        if note:
            ...
            note1 = {"Text": "(Splunk SOAR - {} time is {}) {}".format("Unarchived updated", note_time, note), "UpdatedBy": "automation-splunk"}
    ...
    def _handle_add_note(self, param):
        ...
        note1 = {"Text": f"(Splunk SOAR - {note_time}) {note}", "UpdatedBy": "automation-splunk"}
...
if __name__ == "__main__":
    ...
    if username and password:
        try:
            ...
            r = requests.get(login_url, verify=verify, timeout=AWSSECURITYHUB_DEFAULT_REQUEST_TIMEOUT)
            ...
            r2 = requests.post(login_url, verify=verify, data=data, headers=headers, timeout=AWSSECURITYHUB_DEFAULT_REQUEST_TIMEOUT)
